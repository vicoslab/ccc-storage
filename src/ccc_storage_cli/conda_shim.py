...
from ccc_storage_core.manifest import WRITE_POLICY_SHARED_NFS, normalize_write_policy
...
def _extract_selector(argv: list[str], env: EnvMap) -> str:
    explicit = env.get("CCC_STORAGE_ENV_SELECTOR", "").strip()
    if explicit:
        return explicit
...
def _shim_run_dir(env: EnvMap) -> str:
    ...
    return f"/tmp/ccc-storage-{os.getuid()}"
...
def run_shim(
    ...
    *,
    ...
    if _is_truthy(current_env.get("CCC_STORAGE_SHIM_DISABLE")):
