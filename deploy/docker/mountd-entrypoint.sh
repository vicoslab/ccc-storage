...
CCC_STORAGE_MOUNTD_CONFIG="${CCC_STORAGE_MOUNTD_CONFIG:-}"
if [ -z "$CCC_STORAGE_MOUNTD_CONFIG" ] && [ -f /etc/ccc-storage/mountd.toml ]; then
  CCC_STORAGE_MOUNTD_CONFIG=/etc/ccc-storage/mountd.toml
fi

if [ -z "$CCC_STORAGE_MOUNTD_CONFIG" ]; then
  ...
  CCC_NODE_RUN_DIR="${CCC_NODE_RUN_DIR:-/run/ccc-storage}"
...
if [ -n "$CCC_STORAGE_MOUNTD_CONFIG" ]; then
  exec ccc-storage mountd \
    --config "$CCC_STORAGE_MOUNTD_CONFIG" \
